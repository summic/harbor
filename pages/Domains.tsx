  ...
  const filteredRules = React.useMemo(() => {
    return (rules ?? [])
      .filter((rule) => {
        ...
        return rule.value.toLowerCase().includes(term) || rule.group.toLowerCase().includes(term);
      })
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage domain rules only. Group definitions are on the Domain Groups page.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRules.map((rule) => (
                <tr key={rule.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-slate-600">{rule.group}</td>
